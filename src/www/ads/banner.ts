...
const colorToRGBA = (function () {
  ...
  return function (col: string): { r: number; g: number; b: number; a: number } | undefined {
...
export class BannerAd extends MobileAd<BannerAdOptions> {
  ...
  public static config(opts: { backgroundColor?: string; marginTop?: number; marginBottom?: number }) {
    if (cordova.platformId === Platform.ios) {
      const { backgroundColor: bgColor } = opts;
      return execAsync('bannerConfig', [{ ...opts, backgroundColor: bgColor ? colorToRGBA(bgColor) : bgColor }]);
    }
    return false;
  }
